...
{
    internal class DailyNoteLoader
    {
        private List<DailyNote> _notes = new List<DailyNote>();

        private readonly string _folderPath;
        private readonly string _dailyNoteFormatForRegex;

        internal DailyNoteLoader(string path, string dailyNoteFormatForRegex = @"\d\d\d\d-\d\d-\d\d")
        {
            _folderPath = path;
